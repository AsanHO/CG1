	...
	public:
								  ...
								  float &t, float &u, float &v)
		{
			...
			// 평면과 광선이 수평에 매우 가깝다면 충돌하지 못하는 것으로 판단
			 if (abs(dot(dir, faceNormal)) < 1e-2f) return false; // t 계산시 0으로 나누기 방지
			...
			// 조건1 빼기를 하는 벡터가 모두 같다
			// 조건2 시계방향으로 곱해야함
			 const vec3 normal0 = normalize(cross(point - v2, v1 - v2)); // v2뺌
			 const vec3 normal1 = normalize(cross(v0 - v2,point - v2)); // point 뺌
			 const vec3 normal2 = normalize(cross(v1 - v0, point - v0)); // v0 뺌
